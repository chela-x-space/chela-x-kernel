...
use kernel_domain::{
    ...
    ResourceType, RuntimeId, ScopeId, ScopeLevel, ScopeReference, TimeReference, WorkflowId,
    WorkspaceId,
};
use kernel_gateway::{
...
use crate::{
    AdapterApiVersion, AdapterCapabilityDeclaration, AdapterCapabilityReference,
    AdapterCommandIntent, AdapterCompatibilityReference, AdapterIdentity, AdapterIdentityKind,
    AdapterIntentKind, AdapterKind, AdapterQueryIntent, AdapterRequestContext,
    AdapterRequestEnvelope, AdapterRequestId, AdapterResponseEnvelope,
    AdapterResponseStatusReference, AdapterStatusSnapshot, AdapterValidationStatus,
...
pub fn adapter_api_version() -> AdapterApiVersion {
    AdapterApiVersion::new("2026.07.19").expect("adapter version")
}
...
pub fn studio_view_response() -> StudioViewResponse {
    let projection = StudioViewProjection::TopView(Box::new(
        StudioTopViewProjection::new(
            ...
            vec![RuntimeId::new("runtime.primary").expect("runtime")],
            vec![workflow_id()],
            Vec::new(),
...
pub fn application_command_request_envelope() -> ApplicationRequestEnvelope {
    ApplicationRequestEnvelope::command(application_command_intent())
}

pub fn application_command_response_envelope() -> ApplicationResponseEnvelope {
    let gateway_command_response = GatewayCommandResponse::MemoryCapture(Box::new(
        MemoryCaptureDecision::evaluate(&memory_capture_request()),
    ));
    let studio_command_response = StudioCommandResponse::new(
        &studio_command_request(),
        correlation_id(),
    ...
    .expect("response");
    ApplicationResponseEnvelope::new(
        &application_command_request_envelope(),
        ApplicationResponsePayload::Command(Box::new(studio_command_response)),
        ApplicationResponseStatusReference::new("application.command.response.complete")
            .expect("status"),
        application_audit_reference(),
        later_time_reference(),
    )
...
pub fn other_service_request_id() -> ServiceRequestId {
...
pub fn alternate_query_service_request_context() -> ServiceRequestContext {
    ServiceRequestContext::new(
        service_api_version(),
...
pub fn alternate_service_query_intent() -> ServiceQueryIntent {
    ServiceQueryIntent::new(
        alternate_query_service_request_context(),
        service_query_capability(),
        application_query_intent(),
    )
    .expect("query")
...
pub fn service_query_response_envelope() -> ServiceResponseEnvelope {
...
pub fn service_command_response_envelope() -> ServiceResponseEnvelope {
    ServiceResponseEnvelope::new(
        &command_service_request_context(),
        application_command_response_envelope(),
        ServiceResponseStatusReference::new("service.command.response.complete").expect("status"),
        "2026-07-19T00:10:00Z",
    )
    .expect("response")
}
...
pub fn adapter_command_capability() -> AdapterCapabilityReference {
    AdapterCapabilityReference::new(ADAPTER_COMMAND_CAPABILITY).expect("capability")
}

pub fn adapter_query_capability() -> AdapterCapabilityReference {
    AdapterCapabilityReference::new(ADAPTER_QUERY_CAPABILITY).expect("capability")
}
...
pub fn adapter_capability_declaration() -> AdapterCapabilityDeclaration {
    AdapterCapabilityDeclaration::new(
        adapter_api_version(),
        vec![adapter_command_capability(), adapter_query_capability()],
        true,
        true,
...
pub fn adapter_command_intent() -> AdapterCommandIntent {
    AdapterCommandIntent::new(
        adapter_command_request_context(),
...
pub fn adapter_query_intent() -> AdapterQueryIntent {
    AdapterQueryIntent::new(
...
pub fn adapter_command_envelope() -> AdapterRequestEnvelope {
    AdapterRequestEnvelope::command(adapter_command_intent())
}

pub fn adapter_query_envelope() -> AdapterRequestEnvelope {
    AdapterRequestEnvelope::query(adapter_query_intent())
}

pub fn adapter_response_envelope() -> AdapterResponseEnvelope {
    AdapterResponseEnvelope::new(
...
pub fn adapter_status_snapshot() -> AdapterStatusSnapshot {
    AdapterStatusSnapshot::new(
        adapter_api_version(),
        adapter_identity(),
        adapter_capability_declaration(),
        vec![AdapterIntentKind::Query, AdapterIntentKind::Command],
        vec![
            AdapterCompatibilityReference::new("compat.kernel-service.2026.07.19").expect("compat"),
        ],
        service_api_version(),
