...
| `K11-007` | Revenue view remains reference-only over governed enterprise facts. | `crates/kernel-studio/src/studio_revenue.rs` | `docs/kernel-architecture/12-studio-integration-architecture.md` §11 | revenue reference projection without calculation or payment infrastructure | governed facts only | compile gates, tests | `IMPLEMENTED` | `IMPLEMENTED` |
