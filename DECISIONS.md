...
- `ADR-0004`: `K15 External Intake Trust Boundary` — `ACCEPTED` on `2026-07-22`; establishes a transport-neutral and technology-neutral external intake trust boundary positioned before the K14 External Adapter Boundary. K15 separates untrusted external intake from trusted adapter processing and preserves claimed identity, observed source, verified identity references, trust classification, trust evidence references, correlation, audit continuity, and adapter handoff responsibilities. Human Architecture Approval is complete. No implementation is authorized. K15 implementation remains pending explicit Human Implementation Authorization.
